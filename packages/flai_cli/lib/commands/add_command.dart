...
class AddCommand extends Command<int> {
  ...
  Future<int> run() async {
    if (argResults!.rest.isEmpty) {
      ...
      stderr.writeln('Run \x1B[36mflai list\x1B[0m to see available components.');
      return 1;
    }
    ...
    for (final name in installOrder) {
      ...
      if (brickDir != null && Directory(brickDir).existsSync()) {
        stdout.writeln(
          '\x1B[36m>\x1B[0m Installing $name from brick...',
        );
  ...
  String? _resolveBrickPath(String brickName) {
    ...
    final fromCwd = p.normalize(p.join(Directory.current.path, 'bricks', brickName));
  ...
  void _generateStub(String outputDir, BrickInfo info) {
    ...
    final content = StringBuffer()
      ...
      ..writeln("    return const Placeholder(); // TODO: implement $className")
      ..writeln('  }')
      ..writeln('}');
